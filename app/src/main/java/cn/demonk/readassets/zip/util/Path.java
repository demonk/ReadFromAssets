...
public class Path implements Iterable<String> {
    ...
    //    private static final char[][] sSupportList = {
//            "zip".toCharArray(),
//            "file".toCharArray()
//    };
...
//        char[] pathArray = path.toCharArray();
        for (j = 0; j < path.length(); j++) {
            ...
            if (!((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))) {
                if (StringUtils.match(path, j, SCHEME_SUFFIX)) {
                    return j;
                }
            }
        }
...
//        for (char[] s : sSupportList) {
//            if (s.length < pathArray.length) {
//                boolean pair;
//                for (j = 0; (pair = j < s.length) && s[j] == pathArray[j]; j++) ;
//                for(j=0;)
//                if (!pair) {
//                    //非pair表示已经检查完开头了，可以检查接下来的规则://
//                    if (StringUtils.match(path, j, SCHEME_SUFFIX)) {
//                        return i;
//                    } else {
//                        return -2;
